/* wijnen.js — Vers Le Vin
   Flip cards + hamburger nav for the combined Lantignié wijnen page.
   ------------------------------------------------------------------ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cards.forEach(card => {
    // wrap inner content in .wine-flip-inner for preserve-3d
    const front = card.querySelector('.wine-front');
    const back  = card.querySelector('.wine-back');

    if (front && back) {
      const inner = document.createElement('div');
      inner.className = 'wine-flip-inner';
